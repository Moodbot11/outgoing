...
async function addConversation(phoneNumber, content, isAiResponse = false) {
  ...
  let lead = await getLead(standardizedPhoneNumber)
  if (!lead) {
    console.log(`No lead found for ${standardizedPhoneNumber}. Creating new lead.`)
    await addLead({
      phone_number: standardizedPhoneNumber,
      name: "Unknown",
      status: "new",
    })
    lead = await getLead(standardizedPhoneNumber)
  }
    ...
    if (content !== "User audio received" || isAiResponse) {
      ...
      if (isAiResponse) {
        ...
        if (emailMatch && emailMatch[0]) {
          ...
          console.log(`Extracted email from AI response: ${extractedEmail}`)
          await updateLeadEmail(standardizedPhoneNumber, extractedEmail)
        }
      }
    }
  } else {
    console.error(`Failed to create or find lead for ${standardizedPhoneNumber}`)
  }
}
...
async function updateLeadEmail(phoneNumber, email) {
  ...
  try {
    ...
    if (currentLead) {
      ...
      result = await db.run("UPDATE leads SET email = ?, last_updated = CURRENT_TIMESTAMP WHERE id = ?", [
        email,
        currentLead.id,
      ])
    } else {
    ...
    console.log(`Update/Insert query executed. Rows affected: ${result.changes}`)
...
async function getTranscriptions(phoneNumber) {
  const lead = await getLead(phoneNumber)
  if (lead) {
    return await db.all("SELECT * FROM call_transcripts WHERE lead_id = ? ORDER BY timestamp DESC", [lead.id])
  }
  return []
}

async function updateCallTranscript(callSid, transcript) {
  try {
    await db.run("INSERT OR REPLACE INTO call_transcripts (call_sid, transcript) VALUES (?, ?)", [callSid, transcript])
    console.log(`Call transcript updated for Call SID: ${callSid}`)
  } catch (error) {
