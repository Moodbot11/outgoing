...
async function transcribeRecording(filePath, direction, customerNumber) {
  try {
    const transcription = await openai.audio.transcriptions.create({
      file: fs.createReadStream(filePath),
      model: "whisper-1",
      response_format: "text",
      prompt: "This is a phone conversation. The audio may contain customer service related content.",
    })

    console.log(`Transcription (${direction}) for ${customerNumber}:`, transcription.text)

    // Save transcription to database
    await addConversation(
      customerNumber,
      `Transcription (${direction}): ${transcription.text}`,
      direction === "outgoing",
    )
  } catch (error) {
    console.error(`Error transcribing ${direction} audio for ${customerNumber}:`, error)
  }
}
          ...
          case "stop":
            ...
            if (incomingAudioBuffer.length > 0 || outgoingAudioBuffer.length > 0) {
              try {
                ...
                await saveRecording(streamSid, incomingAudioBuffer, outgoingAudioBuffer)
